        ...
        <Formik
          ...
          validate={(valores)=>{
              ...
              /*if(!valores.ID_Asociado){
                ...
                errores.ID_Asociado='La telefono solo puede contener numeros'
              }*/

              // validacion del nombre del antecedente
              if(!valores.nombreAntecedente){
                errores.nombreAntecedente='Por favor ingresa un antecedente'
              } else if(!/^[a-zA-ZÀ-ÿ\s]{1,40}$/.test(valores.nombreAntecedente)){
                errores.nombreAntecedente='El antecedente solo puede contener letras'
              }

              // validacion de fecha diagnisco
              if(!valores.fecha_Diagnostico){
                errores.fecha_Diagnostico='Por favor ingresa una fecha_Diacnosotico'
              } else if(!/^[0-9]+\-[0-9]+\-[0-9]+$/.test(valores.fecha_Diagnostico)){
                errores.fecha_Diagnostico='La telefono solo puede contener numeros'
              }
              // validacion de tratamiento
              if(!valores.tratamiento){
                errores.tratamiento='Por favor ingresa una respuesta'
              } else if(!/^[a-zA-ZÀ-ÿ\s]{1,40}$/.test(valores.tratamiento)){
                errores.tratamiento='La respuesta solo puede contener letras'
              }
          ...
          onSubmit={(valores, { resetForm }) => {
            ...
            .then(response => {
                ...
                if (response.ok) {
                    // Realiza la segunda solicitud para guardar en antecedentesPatologico
            ...
            .then(response => {
                // Verifica si la segunda respuesta es exitosa
                if (response.ok) {
                    // Si ambas solicitudes fueron exitosas, resetea el formulario y realiza otras acciones necesarias
                    resetForm();
                    cambiarFormularioEnviado(true);
                    //desaparece la leyenda de formulario enviado despues de 5 segundos
                    setTimeout(() => cambiarFormularioEnviado(false), 5000);
                } else {
                    // Si la segunda solicitud no fue exitosa, lanza un error
                    throw new Error('Error al guardar datos en antecedentesPatologico');
                }
            })
              ...
              <div className='form_Grid_HM'>
                {/*<div className='title_Form_HM'><h1>Registro de Historial Medico</h1></div>*/}
                  ...
                  <div className='grup-input'>
                    ...
                    <div className='msg_error'>
                      <ErrorMessage name='nombreAntecedente' component={()=>(
                        <div><span>{errors.nombreAntecedente}</span></div>
                      )}/>
                    </div>
                  </div>
                  <div className='grup-input'>
                    <div><label htmlFor="" className='label-name'>Fecha en la que fue diagnosticada:</label></div>
                    <div><Field 
                          type="text" 
                    ...
                    <div className='msg_error'>
                      <ErrorMessage name='fecha_Diagnostico' component={()=>(
                        <div><span>{errors.fecha_Diagnostico}</span></div>
                      )}/>
                    </div>
                  </div>
                  <div className='grup-input'>
                    ...
                    <div className='msg_error'>
                      <ErrorMessage name='tratamiento' component={()=>(
                        <div><span>{errors.tratamiento}</span></div>
                      )}/>
                    </div>
                  </div>
                  <div className='grup-input'>
